...
void print_error (int err_num)
{
	...
	sprintf(errtext,"#system call error: %s (%d", "malloc", ENOMEM);
	from_backend(0,errtext,strlen(errtext));
	sprintf(errtext,": %s)\n", strerror(ENOMEM));
	from_backend(0,errtext,strlen(errtext));
    } else
	sprintf(errtext,"#error: %s.\n", error_msg[err_num]);
	from_backend(0,errtext,strlen(errtext));
}
...
static int check_object (object *obj)
{
	...
	*end = c;
	...
	obj->txt = ptrmcpy (obj->txt, "\033[0m", 4);
...
int eval_any (long *lres, ptr *pres, char **what)
{
    ...
    if (error) {
	if (opt_debug) {
	    tty_printf("#result not available\n");
	}
	...
	if (res.type==TYPE_NUM || res.type==TYPE_NUM_VAR) {
		...
		*lres=val;
		...
		if (error) {
		    ...
		    sprintf(errtext,"#result: %s\n", ptrdata(txt));
			from_backend(0,errtext,strlen(errtext));
		} else {
		    tty_printf("#result empty\n");
		}
	    }
	}
    }
