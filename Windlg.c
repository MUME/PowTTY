...
enum { IDCX_ABOUT = IDC_ABOUT, IDCX_TVSTATIC, IDCX_TREEVIEW, controlstartvalue,
	...
	powwowpanelstart,
    IDC_TITLE_POWWOW,
    IDC_BOX_POWWOW1, IDC_BOXT_POWWOW1,
    IDC_POWPAGERSTATIC,
    IDC_POWPAGEREDIT,
	IDC_POWEDSTATIC, IDC_POWEDITOREDIT,
	IDC_BOX_POWWOW2, IDC_BOXT_POWWOW2,
	IDC_POWCFGSTATIC, IDC_POWCFGEDIT,
	IDC_POWDIRSTATIC,IDC_POWDIREDIT,
	IDC_POWTMPSTATIC,IDC_POWTMPEDIT,
	IDC_POWEXIT,
	IDC_POWLOCAL,
	powwowpanelend,
...
static void init_dlg_ctrls(HWND hwnd) {
	...
	SetDlgItemText(hwnd,IDC_POWPAGEREDIT,cfg.powwowpager);
	SetDlgItemText(hwnd,IDC_POWEDITOREDIT,cfg.powwoweditor);
	SetDlgItemText(hwnd,IDC_POWCFGEDIT,cfg.powwowcfg);
	SetDlgItemText(hwnd,IDC_POWDIREDIT,cfg.powwowdir);
	SetDlgItemText(hwnd,IDC_POWTMPEDIT,cfg.powwowtmp);
	CheckDlgButton(hwnd,IDC_POWEXIT,cfg.powwowexit);
	CheckDlgButton(hwnd,IDC_POWLOCAL,cfg.powwowlocal);
	...
	int i;
	for (i=0;i<22;i++) {
				SendDlgItemMessage (hwnd, IDC_COLOURLIST, LB_DELETESTRING, 0,
				    (LPARAM) colours[i]);
				}
...
*/
        ...
        /* The Telnet panel. Accelerators used: [acgo] svldrbf */
	{
		...
		/* The Powtty Panel. Accelerators used: [acgo] tuk */
        {
	    struct ctlpos cp;
	    ctlposinit(&cp, hwnd, 80, 3, 13);
            bartitle(&cp, "Settings for PowTTY", IDC_TITLE_POWWOW);
//	    if (dlgtype == 0) {
                beginbox(&cp, "Viewer && Editor Executables",
                         IDC_BOX_POWWOW1, IDC_BOXT_POWWOW1);
                staticedit(&cp, "Pager", IDC_POWPAGERSTATIC, IDC_POWPAGEREDIT, 50);
                staticedit(&cp, "Editor", IDC_POWEDSTATIC, IDC_POWEDITOREDIT, 50);
                endbox(&cp);
   //         }
            beginbox(&cp, "PowTTY File Locations",
                     IDC_BOX_POWWOW2, IDC_BOXT_POWWOW2);
			staticedit(&cp, "PowTTY Temporary Directory",
                       IDC_POWTMPSTATIC, IDC_POWTMPEDIT, 50);
			staticedit(&cp, "PowTTY Config Directory",
                       IDC_POWDIRSTATIC, IDC_POWDIREDIT, 50);
            staticedit(&cp, "Settings File",
                       IDC_POWCFGSTATIC, IDC_POWCFGEDIT, 50);
            
			checkbox(&cp, "Auto&Save Powwow config on exit", IDC_POWEXIT);
			if (dlgtype==1) { // do not allow this to be turned off if it's on during an active connection
					statictext(&cp,"   Local editing cannot be toggled during a session",IDC_POWLOCAL);
			} else {
					checkbox(&cp, "Use MUME local editing", IDC_POWLOCAL);
			}
            endbox(&cp);


            treeview_insert(&tvfaff, 0, "PowTTY");
        }
	...
	if (LOWORD(wParam) == IDCX_TREEVIEW &&
		...
		if (!strcmp(buffer, "PowTTY"))
		hide(hwnd, FALSE, powwowpanelstart, powwowpanelend);
	...
	if (GetWindowLong(hwnd, GWL_USERDATA) == 1) switch (LOWORD(wParam)) {
	  ...
	  case IDC_LOGEDIT:
	    ...
	    break;
	  case IDC_POWPAGEREDIT:
		if (HIWORD(wParam)==EN_CHANGE)
		GetDlgItemText(hwnd,IDC_POWPAGEREDIT,cfg.powwowpager,sizeof(cfg.powwowpager)-1);	
		break;

	  case IDC_POWEDITOREDIT:
		if (HIWORD(wParam)==EN_CHANGE)
		GetDlgItemText(hwnd,IDC_POWEDITOREDIT,cfg.powwoweditor,sizeof(cfg.powwoweditor)-1);	
		break;
	  case IDC_POWCFGEDIT:
		if (HIWORD(wParam)==EN_CHANGE)
		GetDlgItemText(hwnd,IDC_POWCFGEDIT,cfg.powwowcfg,sizeof(cfg.powwowcfg)-1);	
		break;
	  case IDC_POWDIREDIT:
		if (HIWORD(wParam)==EN_CHANGE)
		GetDlgItemText(hwnd,IDC_POWDIREDIT,cfg.powwowdir,sizeof(cfg.powwowdir)-1);	
		break;
	  case IDC_POWTMPEDIT:
		if (HIWORD(wParam)==EN_CHANGE)
		GetDlgItemText(hwnd,IDC_POWTMPEDIT,cfg.powwowtmp,sizeof(cfg.powwowtmp)-1);	
		break;

	  case IDC_POWEXIT:
		  if ((HIWORD(wParam)==BN_CLICKED) || (HIWORD(wParam)==BN_DOUBLECLICKED)) {
			 if(IsDlgButtonChecked(hwnd,IDC_POWEXIT)==BST_CHECKED) {
				cfg.powwowexit=1;
			 } else {
				cfg.powwowexit=0;
			 }
	...
	case IDC_POWLOCAL:
		  if ((HIWORD(wParam)==BN_CLICKED) || (HIWORD(wParam)==BN_DOUBLECLICKED)) {
			 if(IsDlgButtonChecked(hwnd,IDC_POWLOCAL)==BST_CHECKED) {
				cfg.powwowlocal=1;
			 } else {
				cfg.powwowlocal=0;
			 }
