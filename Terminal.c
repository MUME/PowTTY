...
// powwow has a whole logging structure set up.. we'll use it
	    ...
	    default:
	        ...
	        *cpos++ = xlat_tty2scr((unsigned char)c) | curr_attr |
		    (c <= 0x7F ? cset_attr[cset] : ATTR_ASCII);
				...
				log_write((char *)&c,1,0); // hook for the powwow logger - this is CLEAN - no ansi!
		break;
	    }
		curs_x++;
	    if (curs_x == cols) {
	...
	if (!windowflag && term_curpos) move_for_pow(term_curpos);
...
static void do_paint (Context ctx, int may_optimise){
	...
	if (term_curpos+cols+1==0) my_term_curpos=1;
		else my_term_curpos=term_curpos;
	
    our_curs_y = curs_y + (scrtop - disptop) / (cols+1+my_term_curpos);
...
void processed_term_out(int done) {
	...
	int doinput=done;
	int isprompt=0;
	int tmplen=telnet_text_len;
	int i;

	if (done && (telnet_text_len>255) && (
			(telnet_text[telnet_text_len-1] >64)// && (telnet_text[telnet_text_len-1] >91)) ||
		//	((telnet_text[telnet_text_len-1] >96) && (telnet_text[telnet_text_len-1] >123))
			)
		) {
		done=0;
		doinput=0;
	...
	if (telnet_text_len>0) {
		if (!doinput) {
			for (i=telnet_text_len;i>0;i--) {
				if (telnet_text[i]=='\n') {
					tmplen=i+1;
					break;
				}
		...
		telnet_text[telnet_text_len]='\0';
		process_remote_input(&telnet_text,tmplen);
		last_printed=telnet_text[tmplen];

		while ((tmplen<telnet_text_len) && (telnet_text[tmplen]=='\n')) {
				tmplen++;
		}

		if (tmplen != telnet_text_len) {
			for (i=0;(i+tmplen<=telnet_text_len);i++) {
				telnet_text[i]=telnet_text[i+tmplen];
			}
					done=0;
					
		}
		
		telnet_text_len-=tmplen;
...
void process_telnet_text(char *data,int len) {
	...
	while (len--) {
		if (telnet_text_len>= INBUF_SIZE) {
			processed_term_out(0);
		}
		telnet_text[telnet_text_len++] = *data++;
	/*
		if (telnet_text[telnet_text_len]=='\n') {
				processed_term_out(0);
		}
		*/
    }
