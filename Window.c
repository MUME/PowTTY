...
static clock_t last_movement = 0;
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show) {
	...
	if (*p) { // != '\0') {
		...
		i--;
	    p[i] = '\0';
	    ret=do_defaults (p, &cfg);
		if (ret==1) {
			sprintf(errtext,"Loaded Profile: %s\r\n",p);
		} else {
			sprintf(errtext,"Invalid Profile Specified: %s\r\nDefault Profile Loaded\r\n",p);
		}
	...
	wndclass.hIcon         = LoadIcon (inst,
					   MAKEINTRESOURCE(IDI_POWTTYICON));
   ...
   SetWindowPos (hwnd, NULL, 0, 0, guess_width, guess_height,
	...
	window_name = icon_name = NULL;
        if (*cfg.wintitle) {
            ...
            sprintf(msg, "%s - PowTTY (PuTTY)", realhost);
            title = msg;
        }
	set_title (title);
	set_icon (title);
...
static LRESULT CALLBACK WndProc (HWND hwnd, UINT message,
	...
	return 0;
      case WM_DESTROY:
		  if (current_session != NULL) save_settings(current_session,TRUE,&cfg);
	PostQuitMessage (0);
	return 0;
      case WM_SYSCOMMAND:
...
#define TO_CHR_Y(y) (((y)<0 ? (y)-font_height+1: (y)) / font_height)
	// for the mouse wheel.. 120 is the 'offset' for it.
#define zDelta (((short) HIWORD(wParam))/120)
	...
	return 0;
	  ...
	  case 0x020A:
		  ...
		  // so scroll the text as needed
		term_scroll(0,-zDelta);
	return 0;
      case WM_MOUSEMOVE:
	/*
	 ...
	 */
	{
		...
		//BILLJ - hook for the powwow command parser?
			ldisc->send (buf, len);
	    }
	}
	return 0;
...
static int TranslateKey(UINT message, WPARAM wParam, LPARAM lParam,
	...
	if (!left_alt) {
	   ...
	   // force this thing! billj
	   // regardless of what they think they want to do
		...
		//		  else                  xkey = 'l';
	       ...
	       case VK_DIVIDE:  //if(cfg.funky_type==2) 
			   xkey = 'o'; break;
	       case VK_MULTIPLY: // if(cfg.funky_type==2) 
			   xkey = 'j'; break;
	       case VK_SUBTRACT: // if(cfg.funky_type==2) 
			   xkey = 'm'; break;
