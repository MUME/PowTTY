...
time_t movie_last;		     /* time movie_file was last written */
...
void log_resize  (int newsize)
{
    ...
    if (echo_int) {
	sprintf(errtext,"#buffer resized to %d bytes%s\n", newsize, newsize ? "" : " (disabled)");
...
static void log_writeline  (char *line, int len, int kind, long msecs)
{
    ...
    if (++len >= datasize) {
	tty_printf("#line too long, discarded from movie/capture buffer\n");
	return;
    }
