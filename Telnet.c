...
static enum {
    ...
    SEENSB, SUBNEGOT, SUBNEG_IAC, SEENCR,REMOTE_EDIT
	  ...
	  case SUBNEG_IAC:
	   ...
	   case REMOTE_EDIT:
			 ...
			 if (msg_len==0) {
				...
				case 'E':
					message_edit (msg_buf,msg_buflen,0,0);
					
					break;
				case 'V':
					message_edit (msg_buf,msg_buflen,1,0);
					break;
...
static void pre_process_telnet(char *buf, int got_len) {
	char tmp_buf[3];
	int tmp_buflen=-1;
	int index=0;
	int seen_cr,i;
  ...
  if (cfg.powwowlocal==1) {

	if (telnet_state==REMOTE_EDIT) {
		do_telnet_read(buf,got_len);
		return;
	}
	while (len--) {
		for (i=0;i<4;i++) {
			tmp_buf[i]=buf[i+index];
		}
		if (index>0) {
			if (buf[index-1]=='\n') seen_cr=1;
			else seen_cr=0;
		}
		if ((memcmp(tmp_buf,"~$#E",4)==0) && (seen_cr || (index==0))) {
			char tmp[20];
			char *tmpbuf;
			index+=4;
			len-=4;

			msg_mode[0]=(char)buf[index];
			index++;
			len--;
			...
			for (i=0;i<21;i++) {
				...
				if ((buf[i+index]=='\n')) {// || (buf[i+index]=='\r')) {
				index+=i;
				len-=i-1; // for some reason I have to use -1 here, or it misses a char at the end of the first chunk
				msg_len=atoi(tmp);
			...
			// mume likes to send a oneline greeting message
			// we need to preserve it for the edit_sess description,
			// but if it's blank, purge it (ie, we're writing a mail Message)
			...
			for (i=0;i<len;i++) {
				tmpbuf[i]=buf[i+index];
			}
			// woo hoo! time to hook into powwow
			telnet_state=REMOTE_EDIT;
			tmp_buflen=0;
			do_telnet_read(tmpbuf,len);
			free(tmpbuf);
			//telnet_state=SEENCR;
			return;
			} 
